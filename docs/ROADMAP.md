...
- ~~**`requirements.txt` has no dev/prod split, so the production image ships the test frameworks**~~ — **closed 2026-08-26 (OPS-1).** The re-review trigger fired exactly as written: a deployment where image size and attack surface are stated requirements (Scaleway). `requirements.in` / `requirements-dev.in` are now the hand-written inputs, `pip-compile` generates both locks, and the Dockerfile installs the runtime lock only. `docker-smoke` gained the assertion this entry asked for — `pytest`, `playwright`, `faker`, `coverage`, `responses` and `freezegun` must not be importable in the image — and it was confirmed to fail against the previous image before being trusted against the new one. **The size claim in this entry was wrong in an instructive way**: the split saved ~180 MB of the 6.60 GB that is actually inside the container, not the bulk of it. Measuring found `nvidia-*` (2855 MB) and `triton` (723 MB) — CUDA tooling reachable by no supported topology, since `docker-compose.yml` reserves the GPU for `ollama` and gives the `app` service no device at all — plus 351 MB of `.mypy_cache` that `.dockerignore` never excluded. That, not the test frameworks, is what makes cold starts slow. The routes out are measured in [the Scaleway plan](localchat_scaleway_deployment_plan.md) §0; the largest of them costs no supply-chain change, because nothing under `src/` imports `torch` in the first place.
