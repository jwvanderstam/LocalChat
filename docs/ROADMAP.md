...
- **`requirements.txt` has no dev/prod split, so the production image ships the test frameworks** — `pytest`, `pytest-playwright`, `playwright`, `faker`, `coverage`, `responses` and `freezegun` all install into the runtime venv. `.dockerignore` excludes `tests/`, so the test *code* is absent while the frameworks that run it are present. Found while verifying the hardened-base migration (#287). **Why it matters more now than before**: the image is otherwise distroless — no shell, no package manager, nonroot — and shipping Playwright into it puts a browser driver inside an image built specifically to have nothing in it. It is also part of the ~9.5 GB that makes cold starts slow, which `docs/localchat_scaleway_deployment_plan.md` §5 flags independently. **Why parked**: the split touches `requirements.txt`, the `Dockerfile`, and all four CI jobs plus the e2e browser install, and it is a build-topology change rather than a fix for an observed failure. **Re-review trigger**: before any deployment where image size or attack surface is a stated requirement — the Scaleway target is the first one. Do it as `requirements.txt` + `requirements-dev.txt`, with `docker-smoke` extended to assert the test frameworks are *not* importable in the image, since that step's name already promises more than it currently checks.
