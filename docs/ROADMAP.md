...
- ✅ Backfill migration `0014_rbac1_backfill_workspace_members.py`, per the decisions below. (Originally numbered 0012, which collided with the existing `0012_hybrid_search_tsvector.py` — alembic silently resolved the duplicate revision id in favour of one file and dropped the other, so the backfill was unreachable on every database until it was renumbered. Verified applied against a live instance afterwards.) There is no `created_by` column on `workspaces` (only `deleted_by`, from CW-2c), so ownership of existing workspaces could not be inferred from data — it had to be chosen.
...
- **Multi-instance concurrency** (`AppState`/`MetricsCollector` in-process state, the Alembic migration race, `SyncWorker`'s duplicate connector polling, the reranker's duplicate `threading.Timer` scheduler) — all are per-process state with no cross-process coordination; each races or silently diverges at >1 worker or >1 replica. **Why parked**: `UVICORN_WORKERS` now defaults to `1` (Dockerfile/docker-compose) — this app is single-instance by design today, so the defect class is unreachable, not fixed. (This entry used to rest its case partly on the Helm chart's `replicaCount: 1`. PG-0 **deleted the chart** on 2026-08-05, which removes the second replica source entirely rather than pinning it — a stronger reason to park, resting on one setting instead of two.) **Re-review trigger**: before ever running >1 worker or >1 replica, this whole class needs a coordination layer (shared cache backend, a distributed lock for migrations/sync/scheduler, or a real metrics aggregator).
...
- **`STATE_FILE` (`app_state.json`, `src/config.py`) resolves relative to the working directory, not to a guaranteed-writable path.** `AppState.set_active_model()` / `set_rag_param()` write it; `_save_state()` catches and logs any failure rather than crashing, so every failure mode here is silent-degrade.
  **Rewritten 2026-08-27 — the evidence this entry was built on no longer exists.** It argued from the Helm chart's `readOnlyRootFilesystem: true`, concluded that "on a real Helm deployment today, every write fails", and set its re-review trigger as "before the Helm chart is used for a real deployment". **PG-0 deleted the Helm chart on 2026-08-05** (`8294604`), three weeks before this was last edited. The failure it described is now unreachable and the trigger it named can never fire — an entry that had quietly become undismissable rather than parked.
  **What is actually true under the two supported topologies**, re-derived from the code:
  - **Docker.** `WORKDIR /app`, `/app` is `chown`ed to uid 65532, and nothing sets `read_only`. The write **succeeds**. But the `app` service mounts only `app_uploads:/app/uploads` and `app_logs:/app/logs`, so `/app/app_state.json` sits in the container's writable layer and is **lost on every container recreation** — `docker compose up -d --build`, an image bump, any `docker compose down`. The active model and RAG parameters silently revert to their config defaults.
  - **Host run.** Writes land in the working directory and persist. No failure.
  So the defect survived the chart's deletion, but it is *lost on redeploy*, not *fails on write* — a different symptom, a different trigger, and one that fires on the topology the project actually ships.
  **The fix is unchanged, and the reasoning for it is stronger.** `AppState` is shared mutable config; a writable path or a volume mount gives persistence without coherence. The destination is a small `app_settings` table in the Postgres this app already requires, read and written through `src/db/` like every other piece of durable state. That closes the redeploy loss, the multi-instance divergence in the first bullet of this section, and any future read-only-filesystem constraint, instead of patching each symptom separately. **Re-review trigger**: the first time a support question is "why did my active model reset?", or before any deployment where redeploys are routine — which is every containerised one, so this is closer to scheduled than parked.
  **The generalisable point**, and why the correction is recorded rather than quietly applied: this entry was *verified when written* — it says so, "verified while writing this entry" — against a file that was deleted three weeks later. Accepted-debt entries are the documents least likely to be re-read, and the most likely to outlive their evidence. Ch. 18's rule applies to a debt register as much as to a ticket.

- **`gunicorn==26.1.0` is a runtime dependency nothing invokes** — found by the 2026-08-27 documentation audit. It is declared in `requirements.in`, compiled into `requirements.txt`, and therefore installed into the production image; no `Dockerfile`, `docker-compose.yml` service, workflow or module runs it. Everything is uvicorn: the `app` service, all three `mcp-*` services, and the `Dockerfile`'s `CMD`. The leftover is from HK-4, which ported the MCP servers off gunicorn and left the dependency behind — the same commit whose docs kept saying "gunicorn" in three `file-map.md` rows and one `architecture.md` line until this audit. `config.py` still reads a `GUNICORN_TIMEOUT` no code consumes. **Why parked**: removing it is a one-line change to `requirements.in` plus a recompile, but a recompile must happen on Linux (see CLAUDE.md) and this is worth doing with the next dependency change rather than on its own. **Re-review trigger**: the next `pip-compile` run for any reason — drop it then, and `GUNICORN_TIMEOUT` with it.

- **Two PBKDF2 implementations** (`Werkzeug==3.1.8` is a direct dependency existing solely for `generate_password_hash`/`check_password_hash` in `src/db/users.py`, while `src/security_fastapi.py` hand-rolls the same PBKDF2-SHA256 construction via `hashlib` for the admin fallback account) — one primitive, two implementations, one of them justifying a whole dependency. Found by the 2026-08-19 external audit. Both are sound: random salt, `hmac.compare_digest` on the hand-rolled side. **Why parked**: consolidating means touching password hashing to remove a dependency, and getting it wrong locks every user out for no security gain. Worth noting HK-4 ("eliminate Flask") named Flask, Flask-JWT-Extended, Flask-Limiter and Flask-CORS but not Werkzeug, and HK-6's regression gate greps only for `flask` — so a `werkzeug` import passes it. **Re-review trigger**: the next time either hashing path changes for another reason, or the dev/prod dependency split above is done and the runtime dependency list gets a proper audit.
