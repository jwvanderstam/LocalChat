...
8. **Migrations are executed, not merely written** — CI applies the full chain to an empty database, proves it idempotent, and fails on a broken or duplicated revision. No migration reaches a tag having never run. (TQ-5a ✅ — `tests/unit/test_migration_chain.py`, in the fast suite, so `unit-tests` carries it; TQ-5b ✅ — `tests/integration/test_migrations_apply.py`, on a database it creates and drops, so `integration-tests` carries it. Both green on `main` @ `ac51552`.)
