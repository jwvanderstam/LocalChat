...
| `src/security_fastapi.py` | JWT (`python-jose`), rate limiting (`slowapi`), CORS (Starlette middleware); `resolve_principal()` — the one place a caller is identified: decode, fail-closed revocation, and the role read from the database |
